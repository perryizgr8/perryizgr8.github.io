...
---
In my quest to employ my Raspberry Pi to some use, I thought why not track ping roud trip times to various popular websites. This would be less likely to run into the resource constraints that ruined my [speed test project](2020-09-20-monitoring-speed-rpi.markdown), since the Pi's network link wouldn't be saturated. Also I decided to use `go` this time instead of `python`, for performance.
